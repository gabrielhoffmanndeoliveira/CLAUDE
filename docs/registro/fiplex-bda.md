...
- t12 (23 Sep), **Comba RX14V3-AP0-V2AU2X:** the Store API's 140 products include no BDAs. `wp/v2/media?search=V3` works (bogus 0, known-good 2) and leads to `RXxxV3-NG-150450MHz-...-DS-0-0-9.pdf` (05/25): p6 is the full ordering table, p7 decodes filter types (VHF 2A = one DL + one UL window, duplexed; UHF 2X = two windows, custom BW). **The live title was that ordering row cut at 150 characters, before ", 2X", which removed the only differentiator from the 2A/2B/1A siblings.** "AP0" is not decoded. Other RX14V3/RX04V3 listings likely carry the same cut.
