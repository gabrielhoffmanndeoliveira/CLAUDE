...
- e07 (23 Sep): **GWPID-95P** had a truncated-sentence title (the data sheet's General paragraph cut off at "Fire"). CS-2044 Rev B 04/19 p1 (EDAM `datasheets/CS-2044.pdf?download=false`; the bogus CS-2099 gives 8,047 B): "PID-95P: Addressable input interface device mounted on 4.688" plastic plate"; the 70839 trim ring is for flush mounting. This is the **fourth `GW`-prefix case** (GWPID-95P = PID-95P). No document decodes "P", and the standby current is printed as two values with no condition, so it was left out. Mechanism scan queued: titles containing "The <Brand>" after the SKU.
- e09 (23 Sep): **E3ID3-A**, from 9020-0649 Rev X p2 (render). The ordering line "Inner Door, 3 Slots, (NGA, ASM-16 and MIC)" sits under Cabinet "AA". **The same document's p3 capacity table gives one NGA or ASM-16 plus two ASM-16/ANU-48, names no MIC, and labels the row "Cabinet A".** The body quotes both. The ordering block also misprints E3ID2-TA as "E31D2-TA" (digit 1 for letter I).
