...
### e05 (23 Sep 2026): HMS-SA is addressable and Canada-only
...
- e08 (23 Sep): **CRC-6 (500-033250)** is an HNET card for FireFinder XLS, Desigo Fire Safety Modular and Cerberus PRO Modular, not MXL. Data Sheet 8314 Rev 1 (Feb 2023, A6V11208907): six Form C contacts, 4 A at 30 VDC/120 VAC resistive, one CC-5/CC-2 slot. The installation document A6V10239114 resolves to an `/r/<id>/root` map; the text comes from `/api/khub/maps/<mapId>/topics` and then each topic's `contentApiEndpoint`. Warning in it: a slot previously wired for a ZIC-4A can short the PSC-12/PSX-12 24 V output.
- e10 (23 Sep): **TRI-B6M (500-894546)** is **one** single-input module, B6 = chip generation. Data Sheet 6170 (A6V10238827, Jun 2012) p2: 3.5 oz, and TRI-B6MC is the Canada-only version. "MOD. PKG" is ERP text; the $1,867 price is unexplained. Installation sheet 315-049485-3 (A6V10239165) contradicts itself on current (6 mA in the table, 1.8 mA in note 7), so no current was published.
- t09 (23 Sep): **SL2SWW-ALA:** SL2 **wall** appliances are Data Sheet **2601** (A6V14140218, Rev 3, Nov 2025); 2600 (A6V14140217) is ceiling-only. p.4 row: "Strobe WALL Amber WHITE ALERT", so ALA = amber lens + ALERT lettering. Installation sheet P85750-001A (A6V14254253) note 4: amber is not for visual public-mode fire alarm (body copy, not title). A bogus A6V returns the 180 B JSON "unresolved" deeplink. Web-searching model numbers restricted to `sid.siemens.com` surfaces the A6V pages directly.
- t11 (23 Sep): **ST MNS strobes = Data Sheet 2588** (Rev. 2, 03/2023); SET speaker-strobes = 2587 (A6V10333137). ST-CMC-CW-MNS-ALERT (S54301-Z94-A1): ceiling, clear lens, white, ALERT, 15/30/75/95 cd (the wall version goes to 110). A web search surfaced the khub `/content` URL for 2588 directly.
