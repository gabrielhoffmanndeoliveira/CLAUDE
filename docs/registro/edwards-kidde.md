...
- e08 (23 Sep):
  - **CS405-8A-T** is from the harsh-environment series in K85001-0305 Issue 1.2 (not Genesis): a strobe only, 110 cd wall / 60 cd ceiling (UL 1971), 70.7 cd outdoor at -35 °C. **Both imprints use the same number CS405-8A-T**, while Edwards `405-8A-T` is the indoor strobe (Kidde's indoor strobes are 4MS-x). Kidde names the 74347U box and Edwards names 449. Edwards adds "*CS405 models are not FM approved" (left out of the copy). Neither sheet decodes `-T`. **74347U is "Surface Mount Box, Gray, Outdoor" on 0305, but the store title says "Red"** (queued).
  - **1500-7** (K85001-0421 Issue 1.1, identical on the Edwards twin): the ordering row says 5.25–7.5 in.; the p2 drawing says 6.37–8 in. max, with no constant offset. The coordinator removed the range from the title; the body carries both, labelled. Only the extension rods are included. Kidde API categories "Speakers, Strobes, Horns, Bells and Chimes" and "Power Supplies and Accessories" are valid.
- e09 (23 Sep):
